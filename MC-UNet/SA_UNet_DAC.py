...
from DACblock import *
from SPPblock import *
...
def Backbone(input_size=(512, 512, 3), block_size=7,keep_prob=0.9,start_neurons=16,lr=1e-3):
    ...
    return model
def SA_UNet_DAC(input_size=(512, 512, 6), block_size=7,keep_prob=0.9,start_neurons=16,lr=1e-3):
    ...
    conDAC = DACblock(convm,start_neurons * 8)

    # conSPP = SPPblock(conDAC)

    my_concat = Lambda(lambda x: K.concatenate([x[0], x[1]], axis=-1))

    spatial = spatial_attention(convm)

    # convm = conSPP + spatial
    convm = my_concat([conDAC, spatial])

    print(convm)

    convm = Conv2D(start_neurons * 8, (3, 3), activation=None, padding="same")(convm)  #conv4
    convm = DropBlock2D(block_size=block_size, keep_prob=keep_prob)(convm)
    convm = BatchNormalization()(convm)
    convm = Activation('relu')(convm)


    deconv3 = Conv2DTranspose(start_neurons * 4, (3, 3), strides=(2, 2), padding="same")(convm)  #conv4
    # uconv3 = concatenate([deconv3, conv3])
    uconv3 = my_concat([deconv3, conv3])
    ...
    deconv2 = Conv2DTranspose(start_neurons * 2, (3, 3), strides=(2, 2), padding="same")(uconv3)
    # uconv2 = concatenate([deconv2, conv2])
    uconv2 = my_concat([deconv2, conv2])
    ...
    deconv1 = Conv2DTranspose(start_neurons * 1, (3, 3), strides=(2, 2), padding="same")(uconv2)
    # uconv1 = concatenate([deconv1, conv1])
    uconv1 = my_concat([deconv1, conv1])
